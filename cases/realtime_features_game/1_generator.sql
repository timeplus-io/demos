...
(
    -- User identification
    user_id string DEFAULT concat('usr_', to_string(rand64(1) % 20000)), -- Simulates a pool of 20k users
    
    -- Session tracking
    `session_id` string DEFAULT concat('sess_', to_string(rand64(2) % 1000)), -- 1k sessions
    ...
    event_type enum8('match_start' = 1, 'item_pickup' = 2, 'player_elimination' = 3, 'match_end' = 4) DEFAULT multi_if(
        rand(3) % 100 < 10, 'match_start',         -- 10%
        rand(3) % 100 < 50, 'item_pickup',          -- 40%
        rand(3) % 100 < 90, 'player_elimination',   -- 40%
        'match_end'                              -- 10%
    ),
    game_mode enum8('battle_royale' = 1, 'team_deathmatch' = 2, 'capture_the_flag' = 3) DEFAULT multi_if(
        rand(4) % 100 < 75, 'battle_royale',      -- 75%
        rand(4) % 100 < 95, 'team_deathmatch',    -- 20%
        'capture_the_flag'                       -- 5%
    ),
    
    -- Match ID
    `match_id` string DEFAULT concat('match_', to_string(rand(5) % 900 + 100)), -- Like match_789
...
CREATE RANDOM STREAM game_source.transactions (
    ...
    user_id string DEFAULT concat('usr_', to_string(rand64(2) % 20000)), -- Simulates a pool of 20k users
    session_id string DEFAULT concat('sess_', to_string(rand64(3) % 1000)), -- Simulates a pool of 1k sessions
    ...
    transaction_type enum8('iap_purchase' = 1, 'subscription' = 2, 'refund' = 3) DEFAULT multi_if(
        rand(4) % 100 < 80, 'iap_purchase',       -- 80% chance
        rand(4) % 100 < 98, 'subscription',       -- 18% chance
        'refund'                                 -- 2% chance
    ),
    ...
    currency_type enum8('real_money' = 1, 'virtual_currency' = 2) DEFAULT multi_if(
        rand(10) % 100 < 90, 'real_money',        -- 90% chance
...
CREATE RANDOM STREAM game_source.social_events (
    ...
    target_user_id string DEFAULT concat('usr_', to_string(rand(2) % 20000)),
    ...
    interaction_type enum8('message' = 1, 'like' = 2, 'friend_request' = 3, 'party_invite' = 4, 'report' = 5) DEFAULT multi_if(
        rand(3) % 100 < 50, 'message',            -- 50%
        rand(3) % 100 < 75, 'like',              -- 25%
        rand(3) % 100 < 90, 'friend_request',    -- 15%
        rand(3) % 100 < 98, 'party_invite',      -- 8%
        'report'                                -- 2%
    ),
    ...
    additional_data string DEFAULT multi_if(
        ...
        concat(
            '{\"match_id\":\"match_', to_string(rand(5) % 50000), 
            '\",\"message\":\"', array_element(['Good game!', 'GG wanna team up?', 'Nice plays!'], (rand(6) % 3) + 1), '\"}'
        ),
        -- Scenario 2: Reports, which always include a reason
        interaction_type = 'report',
        concat(
            '{\"match_id\":\"match_', to_string(rand(7) % 50000), 
            '\",\"reason\":\"', array_element(['cheating', 'toxic_behavior', 'spam'], (rand(8) % 3) + 1), '\"}'
        ),
        -- Scenario 3: Simple messages in chat
        interaction_type = 'message',
        concat(
            '{\"message\":\"', array_element(['lol', 'anyone for a match?', 'LFG', 'hey'], (rand(9) % 4) + 1), '\"}'
        ),
...
CREATE RANDOM STREAM game_source.performance_metrics (
    user_id string DEFAULT concat('usr_', to_string(rand64(1) % 20000)), -- Simulates a pool of 20k users
    session_id string DEFAULT concat('sess_', to_string(rand64(2) % 1000)), -- Simulates a pool of 1k sessions
    timestamp datetime64(3) DEFAULT now64(3),
    ...
    game_stats string DEFAULT concat(
        '{',
          ...
          '"crash_occurred":', multi_if((rand(4) % 100) < 5, 'true', 'false'), ',', -- ~5% crashes

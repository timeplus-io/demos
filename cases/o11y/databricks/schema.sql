...
CREATE MATERIALIZED VIEW cisco_asa_ta.mv_cisco_asa_logs_single_block AS
SELECT 
  *
FROM cisco_asa_ta.cisco_asa_logs_final
SETTINGS max_insert_block_size=1, max_block_size=1, query_timeout_sec=3000000;
...
SELECT 
  ...
  COALESCE(dest_zone, ''),
  COALESCE(src_zone, '')
FROM cisco_asa_ta.mv_cisco_asa_logs_single_block limit 1

CREATE MATERIALIZED VIEW cisco_asa_ta.mv_cisco_asa_logs_to_databricks
INTO cisco_asa_ta.http_databricks_cisco_asa
AS
SELECT 
  _tp_time,
  message,
  ...
  vendor_product,
  action,
  COALESCE(vendor_action, '') AS vendor_action,
  COALESCE(status, '') AS status,
  COALESCE(transport, '') AS transport,
  COALESCE(protocol, '') AS protocol,
  COALESCE(communication_protocol, '') AS communication_protocol,
  COALESCE(direction, '') AS direction,
  COALESCE(src_interface, '') AS src_interface,
  COALESCE(src_ip, '') AS src_ip,
  COALESCE(src_host, '') AS src_host,
  COALESCE(src_port, '') AS src_port,
  COALESCE(src_user, '') AS src_user,
  COALESCE(src_nt_domain, '') AS src_nt_domain,
  COALESCE(dest_interface, '') AS dest_interface,
  COALESCE(dest_ip, '') AS dest_ip,
  COALESCE(dest_host, '') AS dest_host,
  COALESCE(dest_port, '') AS dest_port,
  COALESCE(dest_user, '') AS dest_user,
  COALESCE(dest_nt_domain, '') AS dest_nt_domain,
  COALESCE(dest_public_ip, '') AS dest_public_ip,
  COALESCE(dest_public_port, '') AS dest_public_port,
  ...
  COALESCE(bytes_in, '') AS bytes_in,
  COALESCE(bytes_out, '') AS bytes_out,
  COALESCE(rule, '') AS rule,
  COALESCE(rule_id, '') AS rule_id,
  COALESCE(rule_name, '') AS rule_name,
  COALESCE(acl, '') AS acl,
  COALESCE(access_group, '') AS access_group,
  COALESCE(icmp_type, '') AS icmp_type,
  COALESCE(icmp_code, '') AS icmp_code,
  COALESCE(assigned_ipv4, '') AS assigned_ipv4,
  COALESCE(assigned_ip, '') AS assigned_ip,
  COALESCE(app, '') AS app,
  ...
  COALESCE(signature_id, '') AS signature_id,
  COALESCE(object, '') AS object,
  COALESCE(object_category, '') AS object_category,
  COALESCE(command, '') AS command,
  COALESCE(process_name, '') AS process_name,
  COALESCE(parent_process_name, '') AS parent_process_name,
  COALESCE(result, '') AS result,
  COALESCE(tcp_flag, '') AS tcp_flag,
  COALESCE(ip_options, '') AS ip_options,
  COALESCE(attack_type, '') AS attack_type,
  COALESCE(group_policy, '') AS group_policy,
  COALESCE(hash_codes, '') AS hash_codes,
  ids_type,
  to_json_string(eventtypes) AS eventtypes,
  to_json_string(tags) AS tags,
  cisco_asa_message_id,
  COALESCE(cisco_asa_user, '') AS cisco_asa_user,
  COALESCE(dest_zone, '') AS dest_zone,
  COALESCE(src_zone, '') AS src_zone
FROM cisco_asa_ta.mv_cisco_asa_logs_single_block;